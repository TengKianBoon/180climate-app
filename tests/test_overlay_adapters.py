...
from core.overlays.pippib import query_pippib, _query_snapshot, _load_snapshot
...
def test_pippib_no_snapshot_returns_none(kalimantan_mineral_boundary, monkeypatch):
    """No PIPPIB_SNAPSHOT_PATH set, no fixture → intersects=None."""
    monkeypatch.delenv("PIPPIB_SNAPSHOT_PATH", raising=False)
    with patch("core.overlays._cache.load_fixture", return_value=None):
        result = query_pippib(kalimantan_mineral_boundary)
    assert result.intersects is None, "INVARIANT: no snapshot → None (not False)"
    assert "PIPPIB_SNAPSHOT_PATH" in result.note


def test_pippib_snapshot_path_env_var_used(pippib_snapshot_path, peat_boundary_hits_snapshot, monkeypatch):
    """PIPPIB_SNAPSHOT_PATH env var activates snapshot query."""
...
# ── Network-marked live smoke test (skipped in CI) ────────────────────────────
