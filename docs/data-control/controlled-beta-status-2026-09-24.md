...
- The shared SQLite file is on the mounted disk. An online backup of the live synthetic records was restored into a separate temporary file and the expected counts were read back. The backup is currently on the same persistent disk, so this does not protect against loss of that disk or establish recurring offsite backups.
...
- Set a private recurring offsite backup destination and recovery schedule. Define and execute the 180-day deletion procedure, including Fieldwork related tables and backup expiry, before the first retention deadline.
